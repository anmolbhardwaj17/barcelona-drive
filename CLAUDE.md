...
- **Tile format**: v11 — v7 (10 feature types) + v8 (baked sidewalks, path clipping) + v9 (baked sky-visibility AO grid; strength dials in `frontend/src/map/aoSampler.js`) + v10 (R-W1 road width SECTION: `carriagewayW` / `parkingLeftW` / `parkingRightW` / `shoulderW` / `kerbToKerbW` / `sidewalkW` / `corridorW`; `width` is an alias of `kerbToKerbW`). **Read it through `frontend/src/map/roadWidths.js` — never re-derive a width.** + **v11 (2026-09-05: IDENTICAL LAYOUT to v10, bumped only to invalidate browser caches after N-25 corrected the coordinate space of ~316,000 baked vegetation positions — a content change no reader can detect). ⚠ THE RULE: bump the version when tile CONTENT changes MEANING, not only when the layout changes. `window._clearTileCache()` is a thing to forget, and it never reaches deployed players at all.**
...
- **Tree cards (v3 P3-10)**: ON by default. `?treecards=0` reverts to the legacy icosahedron blobs — an ATTRIBUTION switch like `?roadv2=0`, because cards trade ~80 triangles per tree for 4 plus an alpha-tested fragment (vertex cost → fill cost). Six photographic Barcelona species from one atlas; species are chosen by CONTEXT (avenue/street/coast/park/plaza), not at random. Re-bake the atlas with `python3 tools/build-tree-atlas.py`.
- **Plaza paving (2026-08-27)**: ON by default. `?plazatex=0` reverts pedestrian plazas to a flat fill. They are panot-paved like the pavements — an ATTRIBUTION switch, because these polygons are large (up to 3,143 m² each) and were invisible until the D-73 LOD fix, so this is the first time anyone has judged them.
- **Pedestrian plazas (diagnostic)**: `?pedareas=0` draws none at all. ⚠ NOT a preference — it deletes real city (six polygons, 6,648 m², largest 3,143 m² in the Gran Via tile alone). It exists because `?plazatex=0` only swaps the MATERIAL and so cannot answer "is that shape a plaza?". One reload settles identity; then argue about how they should look.
...
- **Adaptive resolution**: ON by default, and **ARMED only once the world is drivable** (2026-08-27). It used to tick from frame one, so every probe landed inside the load — where the frame is long because tiles are streaming and no resolution change can reach it. It locked itself out on four consecutive drives, correctly and expensively (4 resizes, 210-519 ms, 15.3 MB). Same mistake the tile-build budget made, D-66. `?adaptres=0` still pins it — an ATTRIBUTION switch, since its resizes cost **70-144 ms each**.
...
| [bake-vs-runtime-audit.md](docs/context/bake-vs-runtime-audit.md) | What the frontend computes every load that the bake could compute once. Measured baseline, per-phase verdict, and why the BIGGEST phase (phys:terrain, 38%) is NOT bakeable. Thesis: bake the decisions, not the vertices |
| [ground-layering.md](docs/context/ground-layering.md) | Co-planar ground stacking — the `groundLayers.js` scheme, the measured audit that found the depth-bias table and the height table encoding OPPOSITE orders for all four paint classes (5 inverted pairs, resolved), and the Z-2..Z-4 backlog of surfaces still outside the scheme. Read before adding any flat ground surface |
| [game-modes.md](docs/context/game-modes.md) | The four modes + Free Roam — the title card and its art contract, the ONE objective halo (and why the old one was a night-only recipe shipped into a day/night game), and the A* road router that replaced crow-flies bearings with Google-Maps-style directions on the map. Read before touching `game/` |
| [pedestrian-system.md](docs/context/pedestrian-system.md) | Pedestrian crowd — how the flipbook bake works, the three measured defects behind "low poly / nothing to do / not smooth" (flat shading, 3 of 11 clips used, one-sub-segment pacing), what P-1 shipped, and the P-2..P-6 plan. Read before touching `pedestrians.js` |
| [audio-plan.md](docs/context/audio-plan.md) | Car sound + ambience TODO (A1-A7, not started). Three engine samples ship but only ONE is ever used; nothing in the game is positional; tunnels sound like open street |
| [backlog.md](docs/context/backlog.md) | **★ THE BOARD — open work on `main`, all streams.** What is blocked on what, what "done" means per ticket, and what has been deliberately DROPPED. `TODO.md` is a pointer to it |
...
| [rapier-physics.md](docs/context/rapier-physics.md) | Rapier (WASM) physics — the ONLY engine. The cannon car and the `?physics=cannon` escape hatch were deleted 2026-07-16 after the release soak; cannon-es survives purely as the collider DESCRIPTOR layer that `rapierWorldAdapter` mirrors. Per-shape streaming collider mirror, runtime-probed heightfield terrain, collision-sound EventQueue |
...
| [tree-cards.md](docs/context/tree-cards.md) | v3 P3-10 photographic tree cards — the atlas bake, the art-bible §4.4 normalize pipeline (`tools/artNormalize.py`), the species-by-context classifier, the billboard collapse, and the two open gate items |
