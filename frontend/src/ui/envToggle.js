...
export function createEnvToggle(refs) {
  ...
  // ── Toggle switch (pill slider with sun/moon icon) ─────────────────────────
  const TRACK_W = 64, TRACK_H = 32, KNOB_SIZE = 26, KNOB_PAD = 3;
  ...
  function updateToggle() {
    ...
    toggle.style.background = isNight ? 'rgba(26, 42, 64, 0.7)' : 'rgba(245, 223, 160, 0.7)';
    knob.style.left = isNight ? `${TRACK_W - KNOB_SIZE - KNOB_PAD}px` : `${KNOB_PAD}px`;
    knob.style.background = isNight ? '#38b6ff' : '#f5b731';
    knob.style.boxShadow = isNight
      ? '0 0 8px rgba(56,182,255,0.5)'
      : '0 0 8px rgba(245,183,49,0.4)';
    knob.innerHTML = isNight
      ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="white"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>'
      : '<svg width="16" height="16" viewBox="0 0 24 24" fill="white" stroke="white" stroke-width="1.5"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"/></svg>';
  }
  updateToggle();
