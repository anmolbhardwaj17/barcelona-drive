...
- 2026-07-09 (v2, Phase 1) — GLB compression. Compressed all 16 in-use models with WebP textures (q90, near-lossless; native in three, no decoder) + meshopt geometry (EXT_meshopt_compression): models 14.7 MB → 3.8 MB (3.8×). Deleted the unused 7.6 MB car.glb (nothing referenced it — the in-game car is bmw_m3.glb). Verified mesh names survive (carModel's Body_* lookups intact) and GLBs validate clean. New src/loaders.js makeGLTFLoader() wires the meshopt decoder; all 5 GLTFLoader sites (carModel, carModels, carShowcase, grassRenderer, bushRenderer) use it. Added @gltf-transform/cli as a devDep. Models dir 22 MB → 8.8 MB overall. Next: KTX2 (GPU texture compression) + vegetation.
