...
- 2026-07-08 — DRIFT overhaul (Space handbrake felt weak): drift now breaks loose at normal city speed and holds a controllable slide. Full slide reached by ~40 km/h (`hbTarget = absSpeed/11`, was `/20`≈72 km/h); drift build lerp 4→6.5 for a snappy tail-out; rear handbrake brake 2.2×→1.25× BRAKE_FORCE so the car KEEPS speed through the slide instead of stopping; yaw-assist full authority by ~58 km/h (`/16`, was `/35`≈126) and DRIFT_YAW_ASSIST 13000→16000. NEW slide-hold mechanic (DRIFT_SLIDE_CAP 1.7 rad/s + DRIFT_SLIDE_HOLD 4200): softly caps yaw rate during handbrake so the tail swings to a big angle and holds instead of spinning out — the "nice drift" feel. All in carPhysics.js.

- 2026-07-08 — DRIFT rebuilt as an ARCADE model (friction-tuning wasn't fun). Space at >14 km/h now enters an authored slide: instead of relying on cannon-es tyre friction, carPhysics steers the chassis velocity vector and yaw DIRECTLY — steering rotates the car (nose leads), momentum chases the nose with a lag that IS the slide angle; grip-recovery ramps past ~60° for natural anti-spin; throttle adds slide speed / brake scrubs it; wheels drop to low frictionSlip so they don't fight the authored slide; release Space snaps grip back. New consts DRIFT_MIN_KMH/YAW_RATE/YAW_LERP/GRIP/SLIP_LIMIT/SCRUB/ACCEL/WHEEL_GRIP. Normal (non-handbrake) grip + lateral damp unchanged, now in the else-branch. Feeds _skidLevel from slip angle → smoke/marks + style DRIFT score.

- 2026-07-08 — DRIFT feel fix #2 ("that's a slide, not a drift"): added OVERSTEER to the arcade drift so the slide is UNSTABLE like a real drift — the slip angle self-generates yaw that GROWS the slide (rear keeps coming around), balanced against a hard-ramping grip recovery. Result: a flick initiates, the drift self-sustains at a big held angle (~42° hands-off), and you counter-steer to hold it on the edge of a spin (the tension that makes it read as a drift, not a slide). New consts DRIFT_OVERSTEER 4.4 + DRIFT_STEER_AUTH 3.2 (split from the old single yaw-rate); DRIFT_YAW_RATE now a spin clamp; DRIFT_SLIP_LIMIT 1.05→0.95, grip ramp 2.2→4.0.

- 2026-07-08 — STYLE tricks now award XP, not money (was earning too much cash from drifts). New game/xp.js singleton (localStorage dd_xp): lifetime style XP + driver LEVEL (ramping curve, level = xpForLevel^1.55). styleSystem banks combos into xp.add() instead of wallet.add() — bank popup shows "+N XP / Lv N" (violet), a LEVEL UP moment with a C-E-G rise sfx + shake. ESC menu Garage now shows 🏅 Level + XP progress bar under the wallet. Cash stays tied to jobs (taxi/delivery/police) only.

- 2026-07-08 — DRIFT DEPTH (build on the new drift): (1) DRIFT CHAIN — holding/linking a slide climbs 6 tiers (DRIFT→NICE→GREAT→AWESOME→INSANE→LEGENDARY) by seconds-held; points accrue at a tier-scaled rate so longer = exponentially more; a 1.25s grace lets you link corners without losing it; the chain banks as one big tier-named DRIFT event into the combo→XP, a wreck forfeits it. Live bottom-centre "DRIFT CHAIN" HUD (tier name+colour, xMult, fill-to-next-tier bar, live points, "keep it lit!"). (2) COLOURED TYRE SMOKE — new game/driftState.js singleton (styleSystem writes tier/colour, carEffects reads) tints the drift smoke by tier (white→yellow→orange→hot-pink→purple) and billows harder at top tiers. All in styleSystem.js + carEffects.js.

- 2026-07-08 — STUTTER fix (regression from drift-depth): (1) the continuous drift rumble added ~0.05 trauma/frame vs ~0.03/frame decay → trauma PINNED at max → the camera shook the full 0.28m every frame for the whole slide (read as stutter). Cut ~6× so it settles low (subtle rumble). (2) near-miss scan now runs every 3rd frame with a REUSED Set (was new Set + arrays every frame at speed → GC). (3) drift-chain HUD gradient string was rebuilt every frame → moved to tier-change only (setDriftTierUI); per-frame only updates points + bar scaleX.

- 2026-07-08 — STUTTER root cause + real fix: the styleSystem screen-shake wrote camera.position DIRECTLY after carCamera positioned it, using Math.random() per frame. Two bugs: (1) random (not smooth sine) = inherently jittery; (2) carCamera cancels only ITS OWN _shakeOffset before its position lerp, so the external offset fed back into the smoothing every frame → persistent stutter that reducing the amount couldn't fix. Removed styleSystem camera shake entirely (addTrauma now a no-op stub; trauma state + MAX_SHAKE_M/TRAUMA_DECAY gone). carCamera already handles impact-punch + high-speed rumble smoothly. All drift features (chain, tiers, coloured smoke, scoring/XP) unchanged.
