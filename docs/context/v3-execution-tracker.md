...
| ~~**N-31**~~ | **sidewalk hangs over nothing — there was no embankment fill in this project at all** | `roadRenderer.js` | ✅ **FIXED 2026-08-29 (edge skirt).** User: *"this road again doesnt have terrain below still we have sidewalk, doesnt make sense right"*. ⚠ **The sidewalk was not the defect** — the pavement really is there; what was missing is the EMBANKMENT. `hasLateralDrop`'s own note lists "every embankment and every cutting" among the cases nothing tags, and the project has never had fill geometry, so an elevated carriageway floats on a plane with terrain metres below. Deleting the pavement would have removed something real; the VOID is the defect, so `emitEdgeSkirt` fills it. **It invents no new policy:** it reuses the guard rail's existing keep MASK (so a skirt appears exactly where a rail does and never crosses a junction) and its `atVals`, which is already the per-point height above terrain — i.e. the drop depth — so there is no extra terrain sampling. Two quads per segment, both windings (a one-sided apron vanishes from half the approaches), depth clamped to 12 m because `edgeAudit` finds a 24.5 m fall in this region and a skirt that deep is a wall. Shares `getGuardRailMaterial()`, so no extra draw call or material switch. |
