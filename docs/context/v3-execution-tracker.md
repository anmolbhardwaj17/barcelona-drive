...
| **N-57** | ✅ **AT ITS DESIGN LIMIT 2026-09-05 — not in flight, and not a defect.** 116 steps (was 133). The "71 declined" premise was the audit's best case (floor is 18), and `[N-65]` says 15 of 15 remaining skips are "a way passes through" — the anchor rule, working as designed. Reopening = revisiting that rule, a design call. |
...
**ANSWERED 2026-09-05 — and the question was wrong, not the pass.** The "71 blendable" figure was an
**upper bound reported as a target**. The audit asked whether the LONGER of the two ways could absorb
the step. The resolver does not get to choose the longer way: `reconcileSharedNodes` picks the ANCHOR
by rule — a way passing through, else a surface way, else a non-tunnel — and bends whatever is left.
When the long way is the anchor, the SHORT way is the mover.

`junctionStepAudit.mjs` now reports the range instead of the best case:

| | of 116 |
|---|---|
| best — the longer way is the mover (the old "71") | **71** |
| split — both sides share it (N-61, needs a genuinely free anchor) | **90** |
| worst — the shorter way is forced to move | **18** ← the floor |

So the pass's real headroom is **18–90, not 71**, and "it is declining 71 steps it could fix" was an
artefact of the question. No code change is warranted on that basis.

**AND THE `[N-65]` BREAKDOWN SAYS THERE IS NOTHING LEFT TO FIX.** Read from the eixample bake:

```
[N-57] 26 nodes disagreed of 14258 — fixed 11, ways moved 11
       | skipped: tunnel 2, too short to blend 15, no geometry 0
       | worst left unfixed 14.2 m, steepest correction 24%, split across both sides 0
[N-65] of the 15 too-short skips — a way passes through 15, more than two ways meet 0,
       tunnel meets surface 0, genuinely too short even across both 0
```

**15 of 15 are "a way passes through".** Not "too short" at all — the label names a symptom, as the
N-65 comment already warned. A way passing through a node cannot bend without putting a kink in the
middle of a road, so it anchors; and because the anchor is not free, N-61's split is unavailable and
the short stub on the other side has to absorb the whole step alone. Zero skips are genuinely short,
zero are many-ways, zero are tunnel-meets-surface.

So the remaining steps are held by a **deliberate design rule, not a defect**. N-57 is at its design
limit. Reopening it means revisiting the through-way rule — deciding that a mid-span kink is
preferable to a 6 m joint — which is a design decision for the user, not a bug fix. ⚠ These figures
are one AREA (26 disagreeing nodes); the region-wide `[N-65]` line rides in every full bake log if
anyone wants the bigger sample before making that call.
...
| **N-22** | ~~sidewalk slabs float ~7.8 m~~ — **NOT ESTABLISHED. My diagnosis was wrong.** | `sidewalkBaker` / runtime | ⚠ **DOWNGRADED 2026-08-28 — do not spend a day on this without new evidence.** The "7.8 m float" came from reading one `_ddPick` as if its hits shared a location. They do not: `_ddPick` lists surfaces **nearest-first along the ray**, and the two hits are **4.36 m apart horizontally** (sidewalk at −147.2/183.0, road at −144.9/179.3). A ray descending a trench bank hits the pavement early and high and the carriageway later and low; the 7.53 m difference is largely the slope between two different places, and the implied 173% grade says the comparison is meaningless, not that the geometry is broken. **The strong evidence points the other way:** baked sidewalk minus nearest road over **1,759 samples is median +0.14 m** — a kerb is 0.15 m — with p05 −0.23 and p95 +0.54. Also ruled out: the sidewalk translate omits `× vertExag` versus the road formula, but `ELEVATION_VERTICAL_EXAGGERATION` is **1**, so they are identical; and terrain uses `scale.y = vertExag, position.y = −off·vertExag`, the same frame. **If revisited, the first move is a probe that samples sidewalk and road at the SAME XZ** — the shape `_ddVegY` uses for vegetation. A pick cannot answer this question. ⚠ This is the FOURTH time "beige pavement" has been diagnosed and been something else (see N-4). |
...
| **N-25** | **trees stand in carriageways — and the ticket was understating it by two orders of magnitude** | `vegetationBaker.js` | ✅ **FIXED 2026-09-05, region re-baked.** ⚠ **The recorded CAUSE was wrong and cost the previous attempt.** It said "baked tree positions are WORLD while `road.points` are MERCATOR, so every lookup falls outside the grid — a silent no-op", generalised from ONE sampled tree. Measured across all 244 tiles (`backend/tools/vegSpaceAudit.mjs`): **97.2% of trees are MERCATOR and match the grid exactly.** The roadside guard worked. That is why the 2026-08-28 attempt deleted 99,715 roadside trees and left the offender count at exactly 4,029 — it added a guard to the path that already had a working one, and the offenders were elsewhere. **THE REAL DEFECT, far bigger than the ticket:** `collectAllPositions` merges two producers — `getRoadsideTreePositions` (reads `road.points`, MERCATOR) and `getBuildingPerimeterTreePositions` (reads `building.footprint`, WORLD) — into one array, and `collectZoneVegetation` emits WORLD throughout. `readBakedVegetation` converts every entry as Mercator. So **316,063 baked positions — ALL 135,228 zone trees, ALL 54,886 zone bushes, 109,963 bushes and 15,986 trees — landed at ≈(-171358, -3797422), 3,800 km off the map. Every tree and bush in every park in Barcelona did not exist.** Two more silent mismatches fell out of the same root: the vegetation MASK rasterised Mercator roads into a WORLD-bounded grid (its road component blocked nothing, ever — N-7's line, still true), and `sortPositionsByDistance` ranked Mercator positions against a WORLD centre, which is what interleaved the spaces inside a single array. **FIX:** normalise the baker to WORLD internally (`convertRoadsForVeg` now converts, which repairs the mask, the ground-road grid, the junction clearance and both sorts at once), convert back to Mercator at emit through ONE helper so no array can be forgotten again, plus `assertVegSpace()` which FAILS THE BAKE rather than relocating a park, and D-23 rejection counters printed by `buildRegion` (a region-wide zero now warns explicitly). **Offender measure rebuilt** — the 4,029 tool was reverted with the change, so `backend/tools/vegRoadAudit.mjs` is the new reference: inside `carriagewayW` on a DRIVABLE road. ⚠ Two earlier cuts were discarded as artefacts: `kerbToKerbW` gives 73% (the baker plants at 5.1-5.5 m against a 5.2 m kerb, so trees straddle it by centimetres) and including footways gives 60,260 (63% of them street trees correctly on pavements). | **RESULT of the 2026-09-05 region re-bake (432 tiles, 520 s):** wrong-space positions in freshly-baked tiles **15,986 → 0** (all four arrays, 1,096,823 positions, 100% Mercator). Trees inside a drivable driving surface **21,897 → 3,305 (-85%)**, p50 depth 0.90 → 0.51 m. Guards region-wide: 15,021 rejected on a ground road · 70,611 inside a building · 80,359 perimeter-masked, and 697 space assertions actually evaluated across 432 tiles. ⚠ **12 STALE ORPHAN TILES remain** (mtime 2026-08-29, listed in `backlog.md`): the current bake does not produce them, so they still carry mixed-space vegetation — every one of the 1,957 residual bad positions is in them. Not deleted: they are real map data at the fringe and K-3 is an open hole ticket.
| **N-26** | **collision wireframes (K) do not appear — and they USED to** | `collisionDebug.js` | ⚠ **OPEN. TWO SPECULATIVE FIXES REVERTED 2026-08-28 — do not guess a third.** The toggle, key binding and HUD all work; no wireframes appear. User: *"we used to see them in green or red"*, so this is a REGRESSION, not a feature that never worked. I changed the parent (`scene` → `worldGroup`) and then added the full physics→local transform, reasoning from a comment rather than from the code that builds colliders; neither helped and both are reverted to `b65400c`. **THE DECIDING DATUM IS THE HUD, WHICH NOBODY HAS READ YET** — it prints `bodies N static M shown S`. `shown 0` means the RANGE FILTER rejects (it compares `camera.position`, scene space, against collider positions built as `px = -(worldX - physicsOrigin.x)`, physics space — and the mismatch grows with the origin offset, which would explain a regression when the spawn moved). `shown > 0` with an empty screen means the group is drawn in the wrong SPACE. Different bugs, opposite fixes. **Get the HUD line before touching it again.** |
...
| ~~**N-5**~~ | urban features shipped `tags: {}` — every OSM tag dropped at the bake | `pbfUrbanFeatures.js` | ✅ **FIXED 2026-08-28.** `extractTags` kept only name/operator/brand. Replaced with a bounded WHITELIST (not `tags` wholesale — an OSM node carries survey dates and source URLs that would ride into every tile forever) of the keys that change what is DRAWN. Measured over a full bake: features with a non-empty tags object **11.1% → 87.9%**, and tile size **unchanged at 149 MB**. ⚠ **The finding this unblocks is bigger than the ticket:** `fire_hydrant:type` now lands on 490 of 537 hydrants and says **473 are UNDERGROUND, only 15 are pillars** — so the game currently draws ~88% of Barcelona's hydrants as the wrong object, a visible red pillar where there is a flush cover plate. Filed as **N-27**. |
| ~~**N-6**~~ | `amenity=drinking_water` was not imported at all | `pbfUrbanFeatures.js` | ✅ **FIXED 2026-08-28.** Barcelona's cast-iron *fonts* now exist in the data: **1,041 features**, against 115 fountains — so they are by far the most numerous street object the game has, and it had none. Variants carried too (49 bubbler, 46 yes, 37 drinking, 1 bottle_refill). ⚠ **Nothing DRAWS them yet and that is deliberate:** `urbanFeatureRenderer` does `if (!builder) continue`, so an unknown type is skipped silently — the data ships with zero visual risk, which is why it was safe to bake unattended. A `drinking_water` builder is **N-28**. |
| ~~**N-27**~~ | **~88% of hydrants are drawn as the wrong object** | `urbanFeatureRenderer.js` | **OPEN, newly measurable thanks to N-5.** `fire_hydrant:type` says **underground 473, pillar 15, wall 2, untagged 47** — and `BUILDERS.fire_hydrant` draws a pillar for all 537. An underground hydrant is a flush cover plate in the pavement, not a standing red post. Cheap: one flat disc builder plus a switch on the tag, and `EXCLUSION_RADIUS` should drop for it (a cover plate clears no street trees). Untagged ones stay pillars — absence of the tag is not evidence of a pillar, but it is the safer default of the two. |
| ~~**N-28**~~ | Barcelona's *fonts* exist in the data but nothing draws them | `urbanFeatureRenderer.js` | **OPEN.** 1,041 `drinking_water` features baked by N-6 and skipped by the builder lookup. The classic Barcelona font is a ~1.1 m cast-iron column with a domed cap and a brass spout; the `fountain`/`bottle` tags separate the column from a modern bubbler. **The single highest object-count-per-model in the game** — one small model would place 1,041 instances of a landmark street object. |
| ~~**N-26**~~ | collision wireframes (K) invisible | `main.js` | ✅ **NOT A BUG — WRONG SERVER. 2026-08-29.** `initCollisionDebug()` sits inside `if (import.meta.env.DEV)` with the comment "Debug overlays are DEV-ONLY: never wire the ?debug= query params or the K-key toggle in a production build". The user was on **4044 = `npm run preview`**, the PRODUCTION build, where the key listener is never registered — hence no wireframes AND no console output. On **4040 = `npm run dev`** it works: HUD reads `bodies 13 static 9 shown 22`. ⚠ **I shipped two speculative fixes against this before checking which build was running, and reverted both.** `_ddPick`/`_ddGround`/`_ddVegCount` are wired OUTSIDE the gate, which is why they worked on 4044 and masked the pattern. **4040 = debug, 4044 = production.** |
| ~~**N-29**~~ | **merge-zone colliders narrowed to 3 m under a 15 m road — the car falls** | `tileManager.js` | ✅ **FIXED 2026-08-29.** Where two corridors overlap the deck collider was clipped, floored at a flat `1.5 m` half-width, whenever the deck sat under `1.5 m` above ground. The clip's own justification is that "near ground the heightfield catches anything we miss" — true when the deck IS the ground, false at 1.4 m up, where a clipped collider drops the car onto terrain or through a gap in it. Two changes, both narrowing WHEN it applies rather than removing it: co-planar threshold **1.5 m → 0.35 m** (kerb height — above that the surfaces are not co-planar and the lip argument fails too), and the floor becomes **60% of the road's OWN half-width** instead of a flat 1.5 m (on a 15 m trunk the old floor left 3 m of collider under 15 m of asphalt, so the outer lanes had nothing). **A lip where two decks overlap is a bump; a missing collider is a fall.** |
| ~~**N-30**~~ | **no railing exactly where there is no ground — the probe read a void as "safe"** | `roadRenderer.js` | ✅ **FIXED 2026-08-29.** `hasLateralDrop` probes 3 m past the kerb and did `if (!Number.isFinite(ty)) continue` — so "no elevation reading beside this lane" was treated as "no fall beside this lane". It is the opposite: a probe that finds NOTHING is a carriageway with a void alongside, the single case most needing an edge. User-reported precisely: *"these roads sit higher than ground or if not ground below them we dont have terrain so railing should be there"*. A missing sample now counts as a drop, provided the road's own height is finite. Only false positive is a rail at the REGION boundary, where there genuinely is no more world — defensible rather than wrong. |
| **N-31** | sidewalk hangs over nothing — **edge skirt tried and REVERTED, needs a sloped batter instead** | `roadRenderer.js` / terrain | ⚠ **OPEN. 2026-08-29.** The diagnosis stands: the pavement is real, the missing EMBANKMENT is the defect, and this project has no fill geometry. The attempted fix does not: `emitEdgeSkirt` dropped a vertical face from the road edge and it read as a **6 m concrete wall beside the road**, worse than the void. User, seeing it: *"do we need these embankments?"* — no. Shipped wrong TWICE, first solid black (no vertex colours against a `vertexColors` material — the second time that contract caught me) and then as a wall. Call site commented out, function kept as a record. **A real embankment is a SLOPED earth batter continuous with the terrain, taking its material and colour from the ground it grows out of — a terrain job, not a road-edge job.** Re-enable only with that; do not switch the line back on. Belongs with N-23: why does terrain not reach these decks at all. |
| ~~**N-32**~~ | **two ends of one street facing each other with no way between — synthesise the missing link** | `OsmDataFixer.js` | ✅ **DONE 2026-08-29, user's idea.** *"we should be smart enough to know here there is a road which is tunnel, otherwise these 2 roads wont merge"* — correct: a real dead end does not point at another dead end. Measured first over the shipped tiles: 3,877 unconnected endpoints, 554 pairs facing each other within 25° at 5–250 m, **41 sharing a street NAME**. `rule7_missingLinkSynthesiser` requires same NAME, same class, both ends genuinely free, each aimed at the other (cos ≥ 0.90), no drivable way CROSSING the line (that is a junction, not a tunnel), and 8–250 m. Emitted as `tunnel`/`layer:-1` so portals, lining and colliders come from the existing pipeline — the rule adds a WAY, not a renderer. **Bake result: `{ freeEnds: 960, pairsConsidered: 266, created: 42, rejectedName: 106681, rejectedAim: 196, rejectedCrossing: 28 }`** — 42 links, against 41 predicted. Precision over recall on purpose: a wrong link is a road through a building, a missed one is only a gap. ⚠ **RE-OPENED then closed by N-37 the same day:** a second bake raised the count to 50, and a probe for them in the shipped tiles found **zero**. The rule was correct; nothing downstream read its output. See N-37. |
| ~~**N-33**~~ | tunnel walls built 40 m tall — a wall of collider standing in open sky | `tileManager.js` | ✅ **FIXED 2026-08-29.** The wall height was `max(abs(eA), abs(eB))` — the DEPTH of the deck below datum, not the height of a wall. A deck 20 m down grew a 20 m wall, centred at `e/2` so half of it stood above the street. User saw them as *"really high collision boxes"*. Now a flat `TUNNEL_WALL_H = 5.0` from the deck up, which is what a tunnel wall is. |
| ~~**N-35**~~ | guard rail runs straight through a merge, walling off two joining lanes | `roadRenderer.js` | ✅ **FIXED 2026-08-29.** User: *"if 2 roads are merging ramp should be removed... where they start merging or reach same height"*. `hasLateralDrop` is a per-ROAD test — a fall ANYWHERE admits the way — and the mask then railed its whole length. The mask now records WHY a road qualified: **structural** (bridge/ramp/layer/trench) stays railed end to end, because the deck really is elevated throughout; **probe-only** keeps a rail per point, per side, only where a fall is actually beside that point. `pointHasDrop` reuses `hasLateralDrop`'s own probe — same offset, threshold and frame, N-30's void rule included — so the two cannot disagree about what a drop is. |
| ~~**N-36**~~ | **a floating roundabout with no barrier at all** | `roadRenderer.js` | ✅ **FIXED 2026-08-29.** User: *"this is a floaring roundabout but ehre we dont have proper ramp on it man, this is risky"*. TWO rules were stripping it, and measuring them separately showed the obvious suspect was the small one: the `closedLoop` reject in `isElevatedGuardRailRoad` accounted for **2** rings; rule 3's proximity zone accounted for **107 elevated roads**, worst with a **24.1 m** fall. Rule 3 is a 2D circle, so a flyover passing 24 m OVER a roundabout lost its barrier for the whole span above it. Zones now carry the ring's deck height and suppress only within 3 m of it, and an elevated ring gets no zone at all. Also measured: `closedLoop` is false on **every** road in the shipped tiles — tile splitting cuts the ring — so that flag was dead weight in both rules. `window._ddRailStats()` proves the rule fires. Probe: `backend/tools/roundaboutRailAudit.mjs`. |
| ~~**N-37**~~ | **the whole OSM fixer was editing a copy nothing downstream ever read** | `buildRegion.js` | ✅ **FIXED 2026-08-29. The session's largest finding.** `buildFromWays` deep-copies every way into a fresh `wayMap`, `fixOsmData` edits that copy, and `buildRoadGeometry` was reading `enriched` — the array from BEFORE the copy. So rule 7's 50 links, rule 5's 9,576 duplicate removals and rules 1/4/6's edits all went nowhere. It survived because NODE edits *did* land (the fixer gets the original `nodeMap` and mutates it in place), so the fixer was half-connected: neither obviously alive nor obviously dead. The D-23 counters were honest about what the fixer DID and silent about what SHIPPED — **a counter at the point of decision does not prove the decision reached the output.** `applyFixerToWays` reconciles the fixed map back, re-running the R-W1 width pass on anything re-classified. ⚠ **Removals stay OFF** (`FIXER_APPLY_REMOVALS=1`): rule 5 wants 20% of the city, and its same-name relaxation was tuned against a metric that could not move. |
| ~~**N-24/N-38**~~ | **ramps with no railing — the end margin was trimming the ends that most needed one** | `roadRenderer.js` | ✅ **FIXED 2026-08-29.** Ramps ARE admitted by `isElevatedGuardRailRoad`, so the rail was not being refused — it was being masked to nothing. Measured over the shipped tiles: **666 ramps, median 48.5 m**, and the 7 m end margin at BOTH ends plus `cleanupKeepMask`'s 10 m minimum run means a ramp needs 24 m of railable length to show anything. **170 of them (25.6%) cannot, at any elevation.** The margin exists so a rail does not protrude past a road's end into whatever it connects to — but at an end that connects to NOTHING there is nothing to protrude into, and an elevated way that simply stops is a cliff edge. Rule 1 now applies per end, only where that end actually meets a junction. Also covers the *"roads chopped with no ramp"* screenshots: a road ending mid-air is now railed to the tip. Counter in `window._ddRailStats()`. |
| ~~**N-27 fix**~~ | hydrant subtypes | `urbanFeatureRenderer.js` | ✅ **FIXED 2026-08-29.** Distribution over the shipped tiles: **underground 473, untagged 47, pillar 15, wall 2**. The builder took no arguments, so all 537 were the same red post — 473 posts standing on pavements that have none. `buildFireHydrant(f)` now branches on `fire_hydrant:type`: underground is a flush cast-iron lid 4 cm proud, wall is a small plate with two nozzles, pillar keeps the post. **Untagged keeps the post** — OSM's silence is not evidence of absence, and a missing object reads as a gap while a wrong one reads as an error. |
| ~~**N-28 fix**~~ | the *font de Barcelona* now exists | `urbanFeatureRenderer.js` | ✅ **FIXED 2026-08-29.** 1,041 `drinking_water` features have been baked since N-6 and nothing drew one. `buildDrinkingFountain` is the standard *model Barcelona*: dark-green cast-iron column on a stone plinth, collar, short curved spout, brass button, domed cap — 1.1 m. Municipal green (`0x2d4034`), not black; the art bible's rule is not to invent a colour when the real one is documented. Densest street furniture in the data after the hydrants, and one of the few objects that reads as Barcelona rather than as generic city. |
| ~~**N-39**~~ | **the FloorValidator caught the synthesised links — twice** | `OsmDataFixer.js` | ✅ **RESOLVED 2026-08-29, bake green.** The N-37 bake failed the commit-blocking `drivable-surface-implies-floor` invariant: 4 violations, both roads `-1000011`/`-1000056` — negative ids, so both were rule 7's own inventions and nothing else in the city. Cause: `buildTrenchCorridors` carves the trench floor as a STRAIGHT LINE between a segment's endpoints while the road is draped on the DEM at every point it has, and a synthesised link had exactly TWO points up to 224 m apart. Real OSM tunnels never hit it — they carry a node every few metres, so their corridors follow the ground. **First fix was wrong in an instructive way:** a node every 15 m with `floor(gap/spacing)` fixed one road and left the other reporting a BYTE-IDENTICAL 0.51 m gap. An identical number is not a partial fix; it means that geometry did not change. Measured: `points 2, len 27.6 m`, and `floor(27.6/15)` is 1, so the loop never ran — the rule's hole was exactly the size of the remaining failure. Now `ceil` with a floor of 2 steps at 5 m, finer than the terrain grid's own cell. **213 nodes added, 0 violations, EXIT=0.** ⚠ No `TRENCH_VALIDATOR=report` bypass and nothing added to `KNOWN_FLOOR_GAP_ROADS` — a guard that fails on geometry this code invented is a guard doing its job. |
| ~~**N-21/N-40**~~ | **two ways drawn on top of each other at full street width** | `OsmDataFixer.js` | ✅ **MEASURED AND SAFE 2026-08-29 — awaiting the user's go for a full bake.** The duplicate remover was gated off because its own counter said 9,576 ways, 20% of Barcelona. A **PAIRED** bake settles it: same district, same bbox, removals on vs off, so nothing but this rule differs → **791 ways deleted, 617 of them `footway`, ~79 drivable, and 2 named streets losing EVERY way.** The "20%" was another counter-versus-artefact gap of exactly the N-37 shape. ⚠ The FIRST diff was confounded and must not be cited: `--area eixample` bakes a smaller bbox, so it reported 1,003 streets vanishing, mostly in Sarrià–Sant Gervasi, which the dedupe never touched. **N-40** adds a live per-name count that refuses any removal taking a street name to zero; verified by bake — `keptLastOfName: 3`, deletions 791 → 789, **streets losing every way: 0**, both casualties present in the tiles. Unnamed ways are not counted: a duplicate unnamed footway is what the rule is for. |
| **N-23** | **surface roads floating up to 25 m above the ground — LOCATED, in the BAKE** | `RampResolver.js` / `buildRegion.js` | 🔍 **DIAGNOSED 2026-08-29, fix not started.** Chased with `_ddNoGround` (runtime) then confirmed against the baked tiles, which is what settled it: **the float is already in the bake**, so the runtime Y-frame theory N-22/N-23 sat on for days is WRONG. Region-wide, surface roads only (layer 0, no bridge, no tunnel), floating > 2 m above baked terrain: **`isRamp` 84/306 = 27.5%, worst 14.12 m; non-ramp 1,669/37,955 = 4.4%, worst 25.11 m.** Being marked a ramp makes a surface road **6× more likely to float** — `RampResolver` is DEM-FREE by design and the tile drape adds ground elevation ON TOP of its profile (`p[3] += groundElev`), so any non-zero profile on an at-grade street lifts it clean off the ground. It is not the whole story: 1,669 non-ramp roads float too and the single worst is one of them. ⚠ Elevation work is governed by [vertical-model-foundation-spec.md](vertical-model-foundation-spec.md) — do not start before reading it. Audit: `backend/tools/rampFloatAudit.mjs`. Note `scanTerrainConflict` (P-R1b V5) already DETECTS this class and deletes nothing; it measured 8.8% of road points, consistent with the 4.4% of roads here. |
| ~~**N-41**~~ | ramp climb spread over a whole street | `RampResolver.js` | ⚠ **SHIPPED, BUT IT DID NOT FIX THE FLOATING — read this before building on it.** Paired over the SAME 306 ways (an unpaired count compares different populations, because this change also altered which roads survive the grade backstop): road points in the air **17.0% → 15.6%**, with **21 ways better, 15 worse, 270 UNCHANGED**. Unchanged dominates because `reach = |Δh| / 0.12` is ~50 m for a 6 m layer step and most ramps are shorter than that, so `reach ≥ L` and the profile is identical to before. **The real win is elsewhere and is large: `[BrokenRamp] 328 → 205`, so 120 roads that were being DELETED from the city now survive** — a distance-based profile has a uniform grade instead of index-fraction spikes between closely-spaced nodes. Keep it for that. The ramp float itself comes from the ELEVATED END sitting at `layer × LAYER_STEP` regardless of terrain, i.e. the layer model, which is governed by [vertical-model-foundation-spec.md](vertical-model-foundation-spec.md). Tools: `rampFloatAudit.mjs` (now reports road-points-in-the-air, not just the peak — the peak lives at the elevated end and cannot move), `pairedRampCompare.mjs`. |
| ~~**N-42**~~ | **1,135 pavements hanging over the daylighted trenches with no deck and no barrier** | `trenchAuthor.js` | ✅ **FIXED 2026-08-29, bake green.** The two CROSSING passes borrowed `DRIVABLE_TUNNEL_TYPES`, which answers a different question. Drivable-only is right for the CARVE — cutting an open trench for a pedestrian subway would gouge the street — and wrong for the crossing test, where a pavement over a hole is still over a hole. Result, two-sided on purpose: **path family `0 → 1,409`** (footway 1031 · cycleway 153 · pedestrian 107 · service 68 · steps 35 · path 14 · busway 1) while **DRIVABLE stayed at 729 → 729, every type delta +0** — a drivable jump would have meant the carve set was widened by mistake. `crossesTrench` also feeds `isElevatedGuardRailRoad`, so these get barriers as well as decks. Side effect checked, not assumed: shoulder cuts 2,337 → 6,508 and they DO carve terrain, but `carveTrenchesIntoGrid` writes only `if (cur > targetY)` — strictly min-only — so ground is lowered, never raised, and only where it stands above the way's own level; carve volume +22% (234,275 → 286,912 cells). FloorValidator ✅ 0 violations. |
| ~~**N-44**~~ | **a slip road that dies beside the road it should join** (rule 8) | `OsmDataFixer.js` | ✅ **SHIPPED 2026-08-29, bake green.** Rule 7 joins two free ends FACING each other; a MERGE is a different shape and rule 7 is structurally blind to it — the stub ends ALONGSIDE the carriageway, pointing the same way, landing on its FLANK. Rule 8 connects it to the target's nearest EXISTING node (never splits the target: splitting mutates a way rule 5, the ramp resolver and the trench flagger have all already reasoned about) and inherits the STUB's class (a service road joining a trunk is still a service road). At-grade only — layer/bridge/tunnel equality is the height test, since the fixer has no DEM; the ones needing a real ramp belong with the layer-model work. **Paired measurement, corrected join test: stubs dying beside a flank 156 → 67, of those at-grade 125 → 44.** 173 connectors created, 220 synthesised ways total in the tiles with rule 7's 49. FloorValidator ✅ 0 violations. ⚠ TWO measurement traps on the way, both worth knowing: rule 8 first inherited rule 7's `DRIVABLE_FOR_LINK`, which excludes `service` — most of the population — and every rejection counter read ZERO while yield was far below the measured population, which is the signature of candidates never arriving rather than filters being strict. Then the audit itself tested only ENDPOINT proximity, so it could not see its own fix (rule 8 joins INTERIOR nodes) and reported 58 of 173 connectors. Tool: `backend/tools/missedMergeAudit.mjs`. |
| ~~**N-45/N-47**~~ | **tunnel portals: the street and the tunnel ramped INTO each other** | `RampResolver.js` | ✅ **FIXED 2026-08-29.** Drivable steps at a shared node **462 → 142**, surface-meets-tunnel **366 → 71 (−81%)**, and the one nobody predicted: **`BrokenRamp` 171 → 29**, so ~142 roads that were being DELETED from the city for impossible grades now survive. Root cause found only by dumping every way at a breaking node: `node 2919136071 [149036726@-24.0 1000038393(tun)@0.0]` — the TUNNEL was already at 0 and the way at −24 was **not** a tunnel. Both ramped at that node in opposite directions because each targeted the other's BASE height instead of its actual profile. N-47: a way at grade (no bridge, no tunnel, layer 0) ignores tunnel neighbours when picking a ramp target — the tunnel rises to meet the street, never the reverse. N-45 additionally made the portal descent distance-based (Case B still used index fraction, N-41's defect, never applied here) and descend AT THE MOUTH, not near the deep end. ⚠ **Four hypotheses, three wrong, each killed by a measurement:** the depth model (78% of tunnels are at a sane −6 m; only 4 ways in the city are at −24 m, and the LOCKED spec lists `LAYER_STEP=6` as correct), the endpoint-target question (N-46 — moved the count by **exactly zero**, the signature of no effect), and mid-tunnel joins (36 of 37 breaks are at an end). |
| ~~**N-48**~~ | **no visible mouth where a trench goes under cover — BUILT, but the population is 2** | `tunnelRenderer.js` | **OPEN, user-reported and reproduced.** The road descends into the open cut and dead-ends in a terrain wall. Per `authored-tunnels-design.md` an open-cut trench is carved INTO the elevation grid while a covered section keeps UNTOUCHED grid — and terrain is a heightfield, which cannot contain a hole. So a covered tunnel's mouth can never be an opening in the terrain; it must be geometry drawn at that boundary. That geometry does not exist. Distinct from N-47 (which makes road and tunnel MEET) and from the residual floaters. **⚠ BUILT 2026-08-29 AND THEN MEASURED — the wrong order, and the measurement deflates the ticket.** The detector works (cover overhead = ground − deck; open cut ≈ 0, covered ≥ 1.5 m; the sign flip is the mouth) but classifying every drivable tunnel gives **fully OPEN 247 · fully COVERED 9 · MIXED 2**. Option L daylights essentially the whole network, so there are only **two** open→covered transitions in the city: `?spawn=41.37131,2.16354` (Carrer del Poeta Cabanyes, cover 1.2 → 6.0) and `?spawn=41.37840,2.18905` (Carrer dels Pescadors, 6.0 → 0.0). **This is NOT the defect the user keeps photographing** — that is a road ENDING on open grass with no cut and no continuation, which belongs to N-49. Population-before-fix was skipped on the one ticket I had recommended as highest value. |
| ~~**N-49 (rule 9)**~~ | **named streets stopping dead — the 5 worst in the city, fixed** | `OsmDataFixer.js` | ✅ **RULE 9 SHIPPED 2026-08-29.** User asked to identify the wrong dead ends BY NUMBERS first — the order N-48 skipped. `deadEndTriage.mjs` scores every unjoined drivable end: class × 2 (motorway stopping = 3, service = 0) **+ 4 if the same street name resumes beyond the gap** + width + length. Of 1,176 ends: 69 ≥ 8, 29 ≥ 10, and a 542-strong tail at score 1 that is ordinary cul-de-sacs. Rule 9 joins a free end to the nearest node of a SAME-NAMED way within 90 m, 53° forward cone, same class, nothing crossing. The name is what buys the reach — rule 8 stops at 12 m because proximity is all it has. **Result: score 14/13/13/12/12 all gone (Gran Via, Autovia de Castelldefels, Meridiana, Isaac Newton, Rambla de Prim); ≥10 29 → 19; ≥8 69 → 49; and the cul-de-sac tail 542 → 540, i.e. UNTOUCHED** — which is the number that proves it fixed broken streets without welding correct ones. 199 created (I predicted 10-40 and was wrong: I anchored on the 11 top-scoring ends, but the rule applies to every free end with a same-named continuation). `rejectedClass 5752`, `rejectedCrossing 74`. ⚠ Still open: the different-name band — `Ronda de Dalt` (15 m gap) and `Ronda Litoral` (72 m) top the list now and need a weaker evidence bar than a name match. The original N-49 note (tunnel-to-tunnel depth disagreements, and the Glòries case where a surface road meets a tunnel's DEEP end because Case B ramps only one end) is UNRELATED and still open. |
| ~~**N-51**~~ | **pillars under elevated roads — WORKING, user-confirmed** | `roadRenderer.js` | ⚠ **PARKED 2026-08-29, instrumented.** User: pillars should exist for everything elevated, not just OSM `bridge=yes`. Measured: drivable decks >2 m above their terrain are **bridge 50 ways / trench deck 682 / ramp 39 / other 1 — 46 km of unsupported deck against 2.5 km supported**. Gate changed from the `bridge` tag to real height (N-51). Two follow-on bugs found and fixed by instrumenting rather than guessing: the position filters ran BEFORE the height test, so every ground road matched *itself* (`onGroundRoad 499/499`, `tallest 0.0 m` — the counter sat downstream of the thing stopping it); then every spot that did pass height was rejected as on a road, because 30 m spacing in a dense grid always lands on one, so piers now step along the deck ±12 m (N-51c). ✅ **DONE 2026-08-30, user confirms columns on screen** (`bridgePillar` visible 13, 6,016 tris; per-tile `built` 31/27/25/23/23/15/14/9/8/5/4/3/1, `nudged 2` so the search is genuinely used). The last bug was the same shape as the first two: **`buildGroundRoadSegments` treats "no bridge tag, no tunnel tag, layer 0" as ground level, and a road lifted by TERRAIN rather than by layer satisfies all three** — so a trench deck was in the ground-road set and every candidate spot matched ITSELF. `nudged 0, built 0` in every tile was the tell: nine offsets over 24 m cannot all be over roadway, so the test was wrong, not the geometry. Segments now carry their way id and `isOnGroundRoad` takes a `skipId` (trench segments widened to the same 6-wide stride, or the shared reader desyncs). ⚠ **Still open:** `inTrench` skips 18-62 per tile — the trench decks, 43 km and the biggest group, where a pier would stand in the open roadway below. Correct to skip; the real fix is ABUTMENT piers at the trench edges, a placement rule rather than a gate. **N-51e/f: BUILT, awaiting a look.** First pass put the pier on the SOLID side of the transition and yielded 1-3 per tile against 18-62 skips — physically wrong, because outside the cut the ground is back at deck level and there is nothing to support. A bridge over a cut takes its load on the BENCH against the trench wall: inside the corridor where the ground has dropped, clear of the carriageway. The pier is now placed on the CUT side, and `buildTrenchCorridorSegments` takes its margin as an ARGUMENT so the spacing loop keeps the generous batter-covering exclusion while the abutment pass uses carriageway + 2.5 m. ⚠ Unverified on screen; the failure mode to watch is a column standing IN the trench carriageway, which would mean 2.5 m is too tight. `?debug=pillars` prints per-tile accounting: roads in / past gate / spots / built / onGroundRoad / inTrench / tooLow / nudged / tallest. |
