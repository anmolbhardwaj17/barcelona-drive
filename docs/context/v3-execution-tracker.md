...
| **initial tile build rate** | — | **~6 tiles in 19.65 s (~3 s/tile)** — the real number behind both `other` and time-to-drive. The work is ASYNC build chunks, which is why the `tiles` lap reads 0.9-2.5 ms while `other` reads 2,000-3,000. **08-27: now REPORTED** — total main-thread ms + chunk count per build phase, printed at the end of the load and carried in the F9 report as `build.phases`. **Read it on the next boot; that names the phase to fix.** | — | **NEXT** |
