...
| **N-1** | **two road surfaces stacked 1 cm apart** at the same XZ — different vertex tints (`#313843` / `#2d333d`) and different AO, both `roadAO+lightGrid`, both depthBias −4 | found by `_ddPick` on Gran Via, 2026-08-27 | ⚠ **UNVERIFIED.** Probably the main carriageway and its lateral legitimately overlapping in OSM — but that is a GUESS, and today's lesson is that "probably fine" surfaces usually are not. NOT the tile duplication (R-J5 fixed that). Cheap: `_ddPick` the spot, then check both road ids in the tile data |
| **N-2** | **254 water polygons replicated into EVERY tile, unclipped** — 4,321,312 m² per tile, incl. a 606,224 m² coastline (1428×3930 m bbox) and "streams" with 6.7 km bboxes, against a 250,000 m² tile | `buildRegion.js` water split | Pure waste, ~0 visual impact (**0%** of road vertices fall inside one, measured). Same class as R-J5 and now cheap to fix the same way: clip the RENDER set, keep the data. Do it in the next bake window |
