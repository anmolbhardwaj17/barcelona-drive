...
| **initial tile build rate** | — | **~6 tiles in 19.65 s (~3 s/tile)** — the real number behind both `other` and time-to-drive. The work is ASYNC build chunks, which is why the `tiles` lap reads 0.9-2.5 ms while `other` reads 2,000-3,000 | — | **NEXT — `takeBuildOverruns()` already records the per-chunk spans; nothing reports them** |
