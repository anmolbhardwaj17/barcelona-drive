...
| **N-31** | sidewalks are drawn where no terrain exists beneath them | `sidewalkBaker` / `roadRenderer` | **OPEN.** User: *"this road again doesnt have terrain below still we have sidewalk, doesnt make sense right"*. Same family as N-30 and N-23 — geometry is emitted against road/kerb geometry without asking whether the ground under it exists. N-30 fixes the SAFETY consequence (a rail now appears); this is the visual one. ⚠ Related to N-23: the underlying question is why terrain is absent under these decks at all, which is the more valuable fix. |
