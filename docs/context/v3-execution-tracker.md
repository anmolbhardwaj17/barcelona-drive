...
| **`other` — the largest section in every report** | — | 2,007–3,087 ms across 40 long frames, vs `rend` 237–415. **08-27: attributed — it is BOOT, not GC.** Every top-`other` frame is inside the first 12 s, with no async work and no allocation; after ~30 s there are no long frames at all | — | D-61 |
...
| **initial load — the biggest cost in the game** | ~16-20 s wall, `other` 2,000-3,000 ms | ⚠ **08-27: MEASURED AND THE CAUSE IS NOT WHAT ANYONE ASSUMED.** `3,095 ms of build work took 16,200 ms of wall time` — ~1,180 chunks averaging **2.63 ms, exactly `FRAME_BUDGET_MS = 3`**, each ending in a yield that costs a whole 16.7 ms frame. **The main thread is IDLE for ~84% of the load, waiting for vsync.** It is YIELD-bound, not work-bound: no build phase needs optimising. `LOAD_BUDGET_MS = 12` while the loader is up (one-way latch, drops to 3 ms the instant the first ring completes). **Predicted 3095/12 ≈ 260 frames ≈ 4.3 s — unverified; the next boot prints it.** | — | **verify** |
