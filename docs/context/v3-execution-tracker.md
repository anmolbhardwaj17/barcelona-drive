...
| **R-J2** | **NEW 08-27** — lane paint, sidewalks and kerbs run straight through 11,934 T-junction mouths | `roadRenderer.getJunctionPoints` | ⚠ **NOT the one-argument job it was first filed as.** `includeTees` supplies the missing junctions, but the rail's clip is RADIAL with a radius of the widest road at the node, and that is wrong twice over for these three: (a) it would cut ~35 m out of a primary for every residential side street — the gap must be sized to the SIDE street, and (b) **a T interrupts only ONE side of the through road**, so a radial clip punches a hole in the pavement and kerb on the opposite side too. Needs side-awareness. A centre line should not break at a T at all — only the edge line on the side the street opens. ~1 day |
