...
| **N-25** | **trees stand in carriageways — and NO road guard can be added to the reverted baker until N-7 is fixed** | `vegetationBaker.js` | ⚠ **OPEN, and the obvious fix is PROVEN NOT TO WORK. Read this before trying it. 2026-08-28.** Attempted the narrow bake-side fix — a kerb-to-kerb `buildPavedGrid` replacing the inset `buildGroundRoadGrid` on the roadside path, plus the first road guard the building-perimeter path has ever had. Baked the region and measured: trees **561,280 → 461,565**, and trees inside a drawn carriageway **4,029 → 4,029 — the identical absolute count.** It deleted 99,715 trees and did not touch one of the offenders. **CAUSE, confirmed in the shipped tile:** baked tree positions are WORLD (`3942.5, 4082.8`) while `road.points` at bake time are MERCATOR (`240798.8, 5069865.5`). A grid built from road points is bounded ~240 km from the trees, so every lookup falls outside it and returns false — **a silent no-op**, the same defect as N-7 ("a mask that blocks nothing is indistinguishable from a mask with generous margins"). N-7's fix was in the REVERTED set. **So this ticket is blocked on the coordinate space, not on the guard.** Anyone re-attempting it must first convert road points to world (N-7's `convertRoadsForVeg`) and PROVE the guard fires with a rejection counter (D-23) before baking. Change reverted; tiles re-baked to the reverted state. |
